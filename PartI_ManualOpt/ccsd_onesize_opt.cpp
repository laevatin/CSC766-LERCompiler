...
double O1[V][O][O][O];
double O2[V][V][V][O];
double tmp1[V][V][V][O][O][O];
double tmp2[V][V][V][O][O][O];
      ...
      for(c=0;c<V;c++)
        for(i=0;i<O;i++)
          for(j=0;j<O;j++)
            for(k=0;k<O;k++)
              for(m=0;m<O;m++)
                tmp1[a][b][c][i][j][k] += T2[a][b][k][m]*O1[c][m][i][j];
      ...
      for(c=0;c<V;c++)
        for(i=0;i<O;i++)
          for(j=0;j<O;j++)
            for(k=0;k<O;k++)
              for(e=0;e<V;e++)
                tmp2[a][b][c][i][j][k] += T2[c][e][i][j]*O2[a][b][e][k];
    ...
    for(b=0;b<V;b++)
      for(c=0;c<V;c++)
        for(i=0;i<O;i++)
          for(j=0;j<O;j++)
            for(k=0;k<O;k++)
              X[a][b][c][i][j][k] += V * tmp1[a][b][c][i][j][k] + O * tmp2[a][b][c][i][j][k];
